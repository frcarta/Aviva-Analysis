...
def nyquist_filter_generator(nyquist_freq, ratio, kernel_size):
    ...
    assert isinstance(
        nyquist_freq, (np.ndarray, list)
    ), "Error: GNyq must be a list or a ndarray"
    ...
    kernel = np.zeros(
        (kernel_size, kernel_size, nbands)
    )  # generic kerenel (for normalization purpose)

    fcut = 1 / np.double(ratio)

    for j in range(nbands):
        alpha = np.sqrt(
            ((kernel_size - 1) * (fcut / 2)) ** 2 / (-2 * np.log(nyquist_freq[j]))
        )
...
def mtf(img, sensor, ratio, mode="replicate"):
    ...
    img_lp = conv2d(
        pad(
            img,
            (h.shape[-2] // 2, h.shape[-2] // 2, h.shape[-1] // 2, h.shape[-1] // 2),
            mode=mode,
        ),
        h,
        padding="valid",
