...
def cayley_dickson_property_2d(onion1, onion2):
    ...
    if dim3 > 1:
        ...
        if dim3 == 2:
            ...
            ris2 = cayley_dickson_property_2d(
                d,
                np.concatenate([np.expand_dims(b[:, :, 0], -1), -b[:, :, 1:]], axis=-1),
            )
            ris3 = cayley_dickson_property_2d(
                np.concatenate([np.expand_dims(a[:, :, 0], -1), -a[:, :, 1:]], axis=-1),
                d,
            )
    ...
    for j in range(stepx):
        for i in range(stepy):
            values[j, i, :] = q_index_metric(
                labels[
                    j * q_shift : j * q_shift + q_block_size,
                    i * q_shift : i * q_shift + q_block_size,
                    :,
                ],
                outputs[
                    j * q_shift : j * q_shift + q_block_size,
...
def SAM(outputs, labels):
    ...
    angle = (
        np.nansum(
            np.arccos(np.clip(scalar_product / norm_product, a_min=-1, a_max=1)),
            axis=-1,
        )
        / norm_product.shape[0]
    )
...
def Q(outputs, labels, block_size=32):
    ...
    for i in range(nbands):
        ...
        numerator = (
            4
            * (N * outputs_labels_sum - outputs_labels_sum_mul)
            * outputs_labels_sum_mul
        )
        denominator_temp = (
            N * (outputs_sq_sum + labels_sq_sum) - outputs_labels_sum_mul_sq
        )
        ...
        quality_map[index] = (
            2 * outputs_labels_sum_mul[index] / outputs_labels_sum_mul_sq[index]
        )
        index = denominator != 0
        ...
        for j in range(search_win):
            rho[i, j, :] = np.mean(
                local_cross_correlation(
                    ms, np.expand_dims(p[i::ratio, j::ratio], -1), floor(ratio / 2)
                ),
                axis=(0, 1),
            )
...
def ReproMetrics(
    outputs, ms, pan, sensor, ratio=4, q_block_size=32, q_shift=32, dim_cut=0
):
    """
...
def DRho(outputs, pan, sigma=4):
    ...
    rho = np.clip(
        local_cross_correlation(outputs, pan, half_width), a_min=-1.0, a_max=1.0
    )
    d_rho = 1.0 - rho
...
###################################################################
# NO REFERENCE METRICS - translated from matlab
from tools.spectral_tools import mtf


def D_lambda_K(fused, msup, ratio, sensor, S=32, degrade=True):
    """
    Spectral distortion index of the Hybrid Quality with No Reference (HQNR).

    Reference:
        [Khan09] M. M. Khan, L. Alparone, and J. Chanussot,
    ...
    ----------
    fused : np.ndarray
        Pansharpened image. Shape: (H, W, C).

    msup : np.ndarray
        Multispectral image upsampled to the panchromatic scale (i.e., same shape as `fused`).

    ratio : int
        Resolution ratio between PAN and MS images.

    sensor : str
        Type of sensor used (e.g., 'QuickBird', 'IKONOS', etc.).

    S : int, optional
        Block size used for Q2^n metric computation. Default is 32.

    Returns
    -------
    Dl : float
        The D_lambda index (spectral distortion measure).

    Raises
    ------
    ValueError
        If input dimensions are inconsistent or not divisible by block size.
    """

    """ #!
    # Check for matching spatial dimensions
    if fused.shape[:2] != msup.shape[:2]:
        raise ValueError("The two images must have the same spatial dimensions.")
    
    N, M, _ = fused.shape
    
    if N % S != 0:
        raise ValueError("Number of rows must be a multiple of the block size S.")
    if M % S != 0:
        raise ValueError("Number of columns must be a multiple of the block size S.")
    """
    if degrade == True:
        # Apply sensor-specific MTF degradation
        # Assuming fused is a NumPy array of shape (H, W, C)
        fused_transposed = np.transpose(fused, (2, 0, 1))  # (C, H, W)
        fused_4d = np.expand_dims(fused_transposed, axis=0)  # (1, C, H, W)
        fused_tensor = torch.from_numpy(fused_4d).float()  # Ensure float32 type

        fused_degraded = mtf(fused_tensor, sensor, ratio)
        fused_degraded = fused_degraded.squeeze(0).permute(1, 2, 0).cpu().numpy()
    else:
        fused_degraded = fused

    # Compute Q2n quality index
    # Q2n_index, _ = Q2n(fused_degraded, msup, S, S)
    Q2n_index = Q2n(fused_degraded, msup, S, S)

    # D_lambda index is 1 - Q2n
...
def r_squared(X, Y):
    """
    Compute R^2 (coefficient of determination) for multivariate linear regression.
    Auxiliary function for D_S
    ...
    # Estimate beta via normal equations
...
def r_squared(X, Y):
    """
    Compute R^2 (coefficient of determination) with safeguards.
    """
    if Y.ndim == 1:
        Y = Y.reshape(-1, 1)

    # Prevent degenerate X
    if np.linalg.matrix_rank(X) < min(X.shape):
        raise ValueError("Regressor matrix is rank-deficient.")
...
def r_squared_ridge(X, Y, lambda_param):
    """
    Compute Ridge Regression coefficients and R^2 (coefficient of determination).

    Args:
        X (numpy.ndarray): The regressor matrix (design matrix).
                           Shape (n_samples, n_features).
        Y (numpy.ndarray): The response vector.
                           Shape (n_samples,) or (n_samples, 1).
        lambda_param (float): The regularization parameter (lambda). Must be non-negative.

    Returns:
        tuple: A tuple containing:
            - numpy.ndarray: The Ridge Regression coefficients (beta_hat_ridge).
                             Shape (n_features, 1).
            - float: The R^2 (coefficient of determination) for the Ridge model.

    Raises:
        ValueError: If lambda_param is negative.
        ValueError: If X or Y are not numpy arrays.
        ValueError: If dimensions of X and Y are incompatible.
    """
    ...
    # Ensure Y is a 2D column vector for consistent matrix multiplication
    if Y.ndim == 1:
        Y = Y.reshape(-1, 1)
    elif Y.ndim > 2 or (Y.ndim == 2 and Y.shape[1] > 1):
    ...
    ridge_term = X.T @ X + lambda_param * identity_matrix

    # Calculate the inverse of the ridge_term
    try:
        inverse_ridge_term = np.linalg.inv(ridge_term)
    except np.linalg.LinAlgError:
        print(
            "Warning: Matrix inversion failed. This might indicate numerical instability."
        )
        raise

    # Calculate beta_hat_ridge = (X.T @ X + lambda * I)^-1 @ X.T @ Y
    beta_hat_ridge = inverse_ridge_term @ X.T @ Y

    # --- Calculate R-squared ---
    Y_hat = X @ beta_hat_ridge  # Predicted Y values from Ridge Regression

    ss_res = np.sum((Y - Y_hat) ** 2)  # Sum of squares of residuals
    ss_tot = np.sum((Y - np.mean(Y)) ** 2)  # Total sum of squares

    # Handle case where Y is constant (ss_tot = 0)
    if ss_tot == 0:
        R2 = 0.0
    else:
        R2 = 1 - (ss_res / ss_tot)

    return R2.item()  # .item() converts 0-dim array to scalar
...
def D_S(fused, pan):
    H, W = fused.shape[:2]
    fused = fused.reshape(H * W, -1)
    pan = pan.reshape(H * W, 1)

    scaler = StandardScaler()
    fused_scaled = np.zeros_like(fused)
    for i in range(fused.shape[1]):
        band = fused[:, i]
        band = band.reshape(-1, 1)
        fused_scaled[:, i] = scaler.fit_transform(band).reshape(-1)  # or ravel()
    # fused = scaler.fit_transform(fused)
    pan = scaler.fit_transform(pan).reshape(-1)  # or ravel()

    # r2 = r_squared(fused_scaled, pan)
    r2 = r_squared_ridge(fused_scaled, pan, 0.1)
    return 1 - r2


# from chatgpt, definition of Q2n index


def q_index_metric(img1, img2):
    """
    Compute Q index between two 2D patches (Zhou Wang 2002).

    Parameters
    ----------
    img1, img2 : np.ndarray
        Input patches of shape (H, W)

    Returns
    -------
    q : float
        Q index value between img1 and img2
    """
    mean1 = np.mean(img1)
    mean2 = np.mean(img2)
    var1 = np.var(img1)
...
def Q2n(image1, image2, block_size=32, stride=32):
    """
    Compute the Q²ⁿ index between two hyperspectral images.

    Parameters
    ----------
    image1, image2 : np.ndarray
        Input images of shape (H, W, B)
    block_size : int
        Size of the spatial blocks (default 32)
    stride : int
        Stride for block extraction (default 32)
    ...
    H_pad, W_pad, _ = image1.shape

    q_map = []

    for y in range(0, H_pad - block_size + 1, stride):
        for x in range(0, W_pad - block_size + 1, stride):
            q_bands = []
            for b in range(B):
                block1 = image1[y : y + block_size, x : x + block_size, b]
                block2 = image2[y : y + block_size, x : x + block_size, b]
                q_bands.append(q_index_metric(block1, block2))
            q_block = np.sqrt(np.sum(np.square(q_bands)))
            q_map.append(q_block)
