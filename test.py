...
from config_dict import config  # epochsconfig
from tools.cross_correlation import local_corr_mask

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")  # e.g., '20250612_142735'

arguments_dict = {
    "input": "Datasets/aviva_eye_aligned_r2_msh53w68b55.mat",
    "ratio": 2,
    # "input": "Datasets\RR2_Milan.mat",
    "out_dir": "Outputs",
    "gpu_number": 0,
    "use_cpu": False,
...
arguments = Arguments(arguments_dict)
...
def test_r_pnn(args):
    ...
    criterion_spec = SpectralLoss(
        ...
        gen_mtf(ratio, sensor, kernel_size=9, nbands=1),
        ratio,
        device,
    ).to(device)
    ...
    for band_number in range(ms.shape[1]):
        ...
        #     ft_epochs = int(
        #         min(
        #             ((wl[band_number].item() - wl[band_number - 1].item()) // 10 + 1)
        #             * config["epoch_nm"],
        #             config["sat_val"],
        #         )
        #     )
